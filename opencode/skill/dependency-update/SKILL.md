...
1. **Check** — Run `$AI_BOOTSTRAP_WORKSPACE/scripts/agent/opencode-deps-check.sh --json` and parse the JSON output. It reports on:
   ...
   - `~/.config/opencode/opencode.jsonc` `plugin` array entries (the script can still read legacy `opencode.json` if a not-yet-migrated config is present)
   - `~/.config/opencode/opencode.jsonc` MCP `command` arrays that reference npm packages (legacy `opencode.json` fallback is read-only discovery)
...
5. **Apply** — Only after approval:
   ...
   - For `opencode.jsonc` `plugin` entries, edit only the matching package version suffix in the reported `location`.
   - For `opencode.jsonc` MCP `command` arrays, edit only the matching command token in the reported `location`.
...
- Never edit `opencode.jsonc` without explicit user approval of the specific changes. If the checker reports a legacy `opencode.json`, ask the user to run the bootstrap update first rather than extending the split-brain state.
- Preserve JSONC formatting and comments in `opencode.jsonc`; use targeted string replacements instead of full-file rewrites.
